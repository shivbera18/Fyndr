...
- **Git:** `init commit` squashed, `shivbera18 <164228363+shivbera18@users.noreply.github.com>` so counted. Small commits, branch PR. Make commits granular and logical. Always use `gh pr review` (never `gh pr comment`) so reviews count as formal GitHub PR reviews.
