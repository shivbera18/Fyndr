...
// Parses yt-dlp `--print "%(id)s | %(title)s | %(uploader)s | %(duration)s"`
// lines: drops malformed rows and long-form filler, caps at MAX_RESULTS.
...
function runYtDlp(args: string[]): Promise<string> {
  return new Promise((resolve, reject) => {
    execFile(YT_DLP_BIN, args, { timeout: SEARCH_TIMEOUT_MS, maxBuffer: 1024 * 1024 }, (err, stdout) => {
      if (err) {
        reject(err);
        return;
      }
